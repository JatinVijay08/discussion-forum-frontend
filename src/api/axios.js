        ...
        if (error.response) {
            ...
            if (status === 401) {
                ...
                if (originalUrl && (originalUrl.includes('/auth/login') || originalUrl.includes('/auth/register'))) {
                    ...
                    window.dispatchEvent(new CustomEvent('unauthorized', { detail: { message: backendMessage } }));
                }
            } else {
            ...
            window.dispatchEvent(new CustomEvent('app-toast', { 
                detail: { title: 'Network Error', message: 'Unable to connect to server. Please check your internet.', type: 'warning' } 
            }));
        }
        return Promise.reject(error);
    }
